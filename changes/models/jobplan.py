...
class JobPlan(db.Model):
    ...
    def get_build_step_for_job(cls, job_id):
        ...
        if job.autogenerated:
            try:
                project_config = job.project.get_config(job.source.revision_sha)
            ...
            if 'bazel.targets' not in project_config:
                logging.error('Project config for project %s is missing `bazel.targets`', job.project.slug, exc_info=True)
                return None, None
