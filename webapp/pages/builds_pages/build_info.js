...
export var SingleBuild = React.createClass({
  ...
  renderButtons: function(build) {
    ...
    let recreateAllLabel = build.containsAutogeneratedPlan ? "Rerun All Targets" : "Recreate Build";
    ...
    if (_.contains(build.tags, 'commit-queue')) {
      ...
      recreateSelectiveTestingButton = <SimpleTooltip placement="left" label="Commit queue builds can't be recreated.">
          <div className="marginTopM"><Button type="white" className={className} disabled={true}>
            <i className="fa fa-repeat marginRightM" />
            Rerun Affected Targets
          </Button></div>
        </SimpleTooltip>;
  ...
  renderDetails: function(build) {
    ...
    if (build.containsAutogeneratedPlan) {
      attributes.push(<InfoItem label="Selective Testing">
